...
def test_modeling_contract_recommendations_use_column_specific_outlier_logic() -> None:
    ingestion_output = {
        "source": "kaggle",
        "competition": "titanic",
        "selected_file": "train.csv",
    ...
    steps = contract["recommended_preprocessing"]

    def _has_step(column: str, operation: str) -> bool:
        return any(item.get("column") == column and item.get("operation") == operation for item in steps)

    assert _has_step("Cabin", "derive_indicator")
    assert _has_step("Cabin", "exclude_column")
    assert _has_step("Name", "exclude_column")
    assert _has_step("Fare", "outlier_treatment")
    assert _has_step("SibSp", "count_feature_treatment")
    assert not _has_step("Pclass", "outlier_treatment")
    assert not _has_step("Parch", "outlier_treatment")
